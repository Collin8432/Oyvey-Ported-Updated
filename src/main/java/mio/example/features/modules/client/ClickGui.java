...
public class ClickGui
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                ...
                Command.sendMessage("Prefix set to " + Formatting.DARK_GRAY + Mio.commandManager.getPrefix());
            }
    ...
    public void onEnable() {
        mc.setScreen(MioGui.getClickGui());
    }
    ...
    @Override
    public void onTick() {
        if (!(ClickGui.mc.currentScreen instanceof MioGui)) {
            this.disable();
        }
    }
